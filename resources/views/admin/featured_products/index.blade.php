...
@section('content')
    <div class="p-3">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('featured_products.create') }}"
                    ...
                    <tbody>
                        @foreach ($products as $prod)
                            <tr class="border-t hover:bg-gray-50">
                                ...
                                <td class="px-5 py-3">
                                    {{ $prod->price !== null ? '$' . number_format($prod->price, 2) : '' }}</td>
                                <td class="px-5 py-3 text-center space-x-2">
                                    <a href="{{ route('featured_products.edit', $prod) }}"
                                        class="bg-yellow-500 text-white px-4 py-1 rounded hover:bg-yellow-600 text-sm">Edit</a>
                                    <form action="{{ route('featured_products.destroy', $prod) }}" method="POST"
                                        class="inline-block" onsubmit="return confirm('Delete this featured product?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
